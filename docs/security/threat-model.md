...
> 上游计划：[`../plan/production-readiness-roadmap.md`](../plan/production-readiness-roadmap.md) Step 03-01 至 03-04。
> 执行说明：本文冻结 Phase 3 安全 contract。Step 03-02 已把 sandbox/resource gate 升级为本地 required release gate；Step 03-03 已补齐 permission/allowlist/decision audit 本地 gate；Step 03-04 已补齐 DID/token lifecycle、challenge replay 和 resolver trust anchor 本地 gate。Step 03-05 至 03-06 仍是必须实现或补强的 Phase 3 release blocker。CI 自动化仍以后续 Phase 6 gate 为准。
...
| capability token | 只在 Host/request boundary 使用，短期有效、可按 scope 验证、可撤销和可按高风险场景做 jti 一次性消费 | JWT claims 绑定 issuer/audience/merchant/user/agent/skill/session/scopes/jti/version；scope derivation 来源固定为 `CapabilityTokenRequest`；`InMemoryTokenLifecycleStore` 支持 revoke、expired prune 和 `ConsumeOnce` replay gate；Debug 输出 redacted | [`token.rs`](../../crates/anp-adapter/src/token.rs)、[`session.rs`](../../crates/anp-adapter/src/session.rs)、[`capability_token_scope.rs`](../../crates/anp-adapter/tests/capability_token_scope.rs)、[`demo_api.rs`](../../crates/demo-server/tests/demo_api.rs) | 当前 lifecycle store 非持久化；生产 token cache/revocation restore、secret store 和跨进程 replay store 由 Phase 4 承接。 |
...
| token scope / audience mismatch | capability claims 和 verifier 检查 scope/audience/merchant/skill/session；token claims version 与 scope derivation source 固定；lifecycle store 支持 revoke、expired prune 和 high-risk `ConsumeOnce` replay gate | [`token.rs`](../../crates/anp-adapter/src/token.rs)、[`session.rs`](../../crates/anp-adapter/src/session.rs)、[`demo_api.rs`](../../crates/demo-server/tests/demo_api.rs) | Step 03-04 本地 gate 已完成；生产持久化 token cache/revocation restore、secret store 和跨进程 replay store 待 Phase 4。 |
...
| DID resolver / token lifecycle 仍为本地内存 gate | 单进程 demo 已能 revoke/logout、jti replay、challenge replay 和 resolver mismatch fail closed；若直接用于多进程生产，revocation/replay 状态无法跨重启恢复 | 中 | Step 03-04 已提供 `CapabilityTokenLifecycleStore`、`InMemoryTokenLifecycleStore`、`ChallengeNonceStore`、`TrustedDidDocumentResolver`、server-side challenge 一次性消费和 focused tests | Phase 4 承接 secret store、持久化 token cache/revocation restore、跨进程 replay store、DID network/rotation 配置；Phase 6 承接 CI/ops gate | `anp-adapter`、`demo-server`、Host adapter | Phase 4/6 | 是，production release 前 |
