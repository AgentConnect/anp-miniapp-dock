...
| scoped storage | DID + merchant + Skill 隔离 | `StorageScope` 与 in-memory storage 测试 | [`storage.rs`](../../crates/wx-compat/src/storage.rs)、[`scoped_storage.rs`](../../crates/wx-compat/tests/scoped_storage.rs) | 当前非持久化；生产 storage encryption、quota、migration 未实现。 |
| outbound network | 默认无任意出站，所有业务网络走 RequestBroker/allowlist | empty allowlist deny；authority allowlist；component 默认 deny request | [`signed_request.rs`](../../crates/anp-adapter/src/signed_request.rs)、[`capability_token_scope.rs`](../../crates/anp-adapter/tests/capability_token_scope.rs)、[`component_permissions.rs`](../../crates/wx-compat/tests/component_permissions.rs) | 当前 Atomic API `wx.request` 仍有 demo-only localhost bridge，Phase 1 必须收敛。 |
...
| 任意网络出站 | RequestBroker allowlist deny by default；component profile 默认 deny request | [`capability_token_scope.rs`](../../crates/anp-adapter/tests/capability_token_scope.rs)、[`component_permissions.rs`](../../crates/wx-compat/tests/component_permissions.rs) | 当前 gate：workspace tests；Phase 1 需替换 demo-only `wx.request`。 |
| JS 覆盖 `Authorization` | API 矩阵明确 Phase 1 必须剥离或拒绝 JS-provided `Authorization` | [`wx-api-compatibility-matrix.md`](../architecture/wx-api-compatibility-matrix.md) | Planned gate：Step 01-04 之后启用。 |
...
|---|---|---|---|---|---|---|---|
| `wx.request` 仍存在 demo-only localhost bridge | 若误用于生产，可绕过统一 broker 策略 | 中 | API 矩阵和基线标为 `demo-only`，release gates 禁止 demo-only 进入 production | Phase 1 必须收敛到 RequestBroker | `js-runtime-quickjs`、`anp-adapter` | Phase 1 Step 01-04 | 是，production release 前 |
