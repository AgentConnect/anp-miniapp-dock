...
| 内置组件 | `input` / `textarea` / `radio` / `checkbox` / `picker` | `supported` | Step 02-04 | `RenderNodeKind::{Input,Textarea,Radio,Checkbox,Picker}`，输出安全 props、`input` / `change` event 和 disabled 状态。 | Host renderer/provider 负责真实输入控件和提交路径。 | 表单值只是 Render IR 数据，不能绕过 input validation、permission 和 consent。 | [`wxml_bindings.rs`](../../crates/component-runtime/tests/wxml_bindings.rs)、address-form fixture/snapshot 待 Step 02-06 | `component-runtime`、Host renderer | 不实现完整输入法事件模型或 form submit。 |
| 内置组件 | `map` preview / `canvas` static | `host-boundary` | Step 02-04 + Step 02-06 | `RenderNodeKind::{MapPreview,CanvasStatic}`；只输出 `region` / opaque location token / poster 等静态 props。 | Host renderer 负责静态预览。 | 不开放 `MapContext.*`、精确经纬度透传、markers、canvas script 或 touch 交互。 | [`wxml_bindings.rs`](../../crates/component-runtime/tests/wxml_bindings.rs)、location-map-preview fixture 待 Step 02-06 | `component-runtime`、Host renderer | 完整地图交互、真实位置 provider 和 canvas script 仍 unsupported。 |
...
| category | capability | status | target_phase | render_ir_mapping | host_boundary | security_notes | fixture_or_snapshot | owner_crate | notes |
|---|---|---|---|---|---|---|---|---|---|
| Render IR | Platform-neutral node tree | `supported` | P0 + Step 02-01 + Step 02-04 | `ComponentRenderOutput { schemaVersion, root, warnings }`；`RenderNode { id, kind, text, props, style, events, children, accessibility }`；P1 node kind registry 已包含 form/static media。 | Host renderer 消费 JSON。 | `debug`/sensitive fields 不应进入生产 Render IR。 | [`render_ir.rs`](../../crates/component-runtime/src/render_ir.rs)、[`wxml_bindings.rs`](../../crates/component-runtime/tests/wxml_bindings.rs)、[`coffee_order_flow.rs`](../../crates/dock-cli/tests/coffee_order_flow.rs) | `component-runtime`、`dock-cli` | 顶层 `schemaVersion` 固定为 `dock.render-ir.v1`；golden snapshots 仍待 Step 02-06。 |
