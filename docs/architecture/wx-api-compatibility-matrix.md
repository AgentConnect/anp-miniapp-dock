...
| 媒体/文件 | `wx.chooseMedia` | 原子接口 | `planned-p1` | Phase 1/3 | Host media picker 返回 opaque handle，不返回真实本地路径；`format: "image"` 输入字段已由 manifest validator 报告为 Host media provider 依赖。 | L4 | `mcp-schema`、Host file/media provider、`consent-audit` | callback/Promise | [`mcp_validation.rs`](../../crates/mcp-schema/tests/mcp_validation.rs) 覆盖 input format warning；API provider 待新增 | 与 `format: "image"` 输入字段联动；真实 chooser 仍待 provider。 |
| 媒体/文件 | `wx.chooseMessageFile` | 原子接口 | `planned-p1` | Phase 1/3 | Host file picker 返回 opaque handle，后续由 file broker 读取；`format: "file"` 输入字段已由 manifest validator 报告为 Host file provider 依赖。 | L4 | `mcp-schema`、Host file provider、`consent-audit` | callback/Promise | [`mcp_validation.rs`](../../crates/mcp-schema/tests/mcp_validation.rs) 覆盖 input format warning；API provider 待新增 | 禁止任意路径读；真实 chooser 仍待 provider。 |
