...
| 网络 | `wx.request` | 原子接口；动态组件需 `scope.dynamic` | `demo-only` | Step 01-04 | QuickJS 已收敛到 `wx-compat::RequestBroker` trait 的本地 DID broker，执行 loopback allowlist、session bearer 注入和 response header redaction。 | L1-L4 by URL/data | `js-runtime-quickjs`、`wx-compat`、`anp-adapter` | HTTP response resolve；broker/local failure reject | [`middleware_chain.rs`](../../crates/js-runtime-quickjs/tests/middleware_chain.rs)、[`capability_token_scope.rs`](../../crates/anp-adapter/tests/capability_token_scope.rs)、[`coffee_order_flow.rs`](../../crates/dock-cli/tests/coffee_order_flow.rs) | JS 传入 `Authorization` / `Signature` / `Signature-Input` / `Cookie` 会 `request:fail permission_denied` 且不出站；非 loopback URL `network_denied`。生产 Host transport、registry allowlist 和 persistent audit 仍待 Phase 4。 |
...
| Storage | `wx.getStorage` / `wx.setStorage` / `wx.removeStorage` / `wx.clearStorage` | 原子接口；组件环境待 Phase 2/4 | `supported` | Step 01-06 | Atomic API VM 已注入 scoped in-memory storage，scope = `userDid + merchantDid + skillId`；空 key、NUL、超限 key/value、敏感 key、quota 和非 JSON-safe value fail closed。 | L2 | `wx-compat`、`js-runtime-quickjs` | async callback + Promise | [`scoped_storage.rs`](../../crates/wx-compat/tests/scoped_storage.rs)、[`middleware_chain.rs`](../../crates/js-runtime-quickjs/tests/middleware_chain.rs) | 当前不持久化，生产 storage encryption、migration、backend quota 和 cleanup 留到 Phase 4；storage 内容不会自动进入 model-visible result。 |
| Storage sync | `wx.getStorageSync` / `wx.setStorageSync` / `wx.removeStorageSync` / `wx.clearStorageSync` | 原子接口；组件环境待 Phase 2/4 | `supported` | Step 01-06 | 同一 scoped in-memory backend；成功直接返回 data 或 `undefined`，失败抛出脱敏 Error，`errMsg` 使用同步 API 名称。 | L2 | `wx-compat`、`js-runtime-quickjs` | sync throw | [`scoped_storage.rs`](../../crates/wx-compat/tests/scoped_storage.rs)、[`middleware_chain.rs`](../../crates/js-runtime-quickjs/tests/middleware_chain.rs) | 协议参考列出 `getStorageSync` / `setStorageSync`，remove/clear sync 按微信常见语义纳入；同步失败不泄露 key/value 原文。 |
...
| 地址 | `wx.chooseAddress` | 原子接口 | `host-boundary` | Step 01-08 + Phase 3/4 | Atomic API 已接入高风险 Host provider boundary；默认 fail closed，dev-only mock 只能返回 opaque token 和 `devOnly/mock` 标识。 | L4 | `wx-compat`、`js-runtime-quickjs`、Host adapter、`consent-audit`、`dock-core` | callback + Promise；失败 reject | [`high_risk_provider.rs`](../../crates/wx-compat/tests/high_risk_provider.rs)、[`middleware_chain.rs`](../../crates/js-runtime-quickjs/tests/middleware_chain.rs) | 不允许 Skill 直接读本机地址簿或地址原文；真实 provider/conformance 留到 Phase 3/4。 |
...
| 位置 | `wx.getLocation` / `wx.getFuzzyLocation` | 原子接口 | `host-boundary` | Step 01-08 + Phase 3/4 | Atomic API 已接入高风险 Host provider boundary；默认 fail closed，dev-only mock 只返回 opaque location token / fuzzy 标识。 | L4 | `wx-compat`、`js-runtime-quickjs`、Host adapter、`consent-audit` | callback + Promise；失败 reject | [`high_risk_provider.rs`](../../crates/wx-compat/tests/high_risk_provider.rs)、[`middleware_chain.rs`](../../crates/js-runtime-quickjs/tests/middleware_chain.rs) | 不返回精确经纬度原文；真实 provider/conformance 留到 Phase 3/4。 |
...
| 媒体/文件 | `wx.chooseMedia` | 原子接口 | `host-boundary` | Step 01-08 + Phase 3/4 | Atomic API 已接入高风险 Host provider boundary；默认 fail closed；dev-only mock 只返回 opaque `fileHandle`，本地路径输入被拒绝且不回显。 | L4 | `wx-compat`、`js-runtime-quickjs`、`mcp-schema`、Host file/media provider、`consent-audit` | callback + Promise；失败 reject | [`high_risk_provider.rs`](../../crates/wx-compat/tests/high_risk_provider.rs)、[`middleware_chain.rs`](../../crates/js-runtime-quickjs/tests/middleware_chain.rs)、[`mcp_validation.rs`](../../crates/mcp-schema/tests/mcp_validation.rs) | 与 `format: "image"` 输入字段联动；真实 chooser 仍待 provider/conformance。 |
| 媒体/文件 | `wx.chooseMessageFile` | 原子接口 | `host-boundary` | Step 01-08 + Phase 3/4 | Atomic API 已接入高风险 Host provider boundary；默认 fail closed；dev-only mock 只返回 opaque `fileHandle`。 | L4 | `wx-compat`、`js-runtime-quickjs`、`mcp-schema`、Host file provider、`consent-audit` | callback + Promise；失败 reject | [`high_risk_provider.rs`](../../crates/wx-compat/tests/high_risk_provider.rs)、[`middleware_chain.rs`](../../crates/js-runtime-quickjs/tests/middleware_chain.rs)、[`mcp_validation.rs`](../../crates/mcp-schema/tests/mcp_validation.rs) | 禁止任意路径读；真实 chooser 仍待 provider/conformance。 |
