...
| Next action | 等待 Step 02-01 完成后，启动组件 manifest metadata runtime flow |
...
- [ ] `relatedPage` 从 manifest 进入 runtime 可观测 metadata/action，并经过 path/query canonicalize。
- [ ] `permissions.scope.dynamic` 进入 component capability profile；未声明 dynamic 的组件默认 deny request/timer。
- [ ] `expirable` 和 `expiredText` 进入 runtime 过期策略或 RenderOutcome metadata，未声明时不被误判为 production expirable。
- [ ] `_meta.ui.componentPath` 与 `components[].path` 的 canonical path 规则一致。
- [ ] 未知 `_meta` 保留在 Host/private 边界，不进入模型可见输出。
